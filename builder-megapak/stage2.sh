...
export PATH="$PATH:$workdir/ComfyUI_MEGAPAK/python_standalone/Scripts"

ls -lahF

# Redirect HuggingFace-Hub model folder
export HF_HUB_CACHE="$workdir/ComfyUI_MEGAPAK/HuggingFaceHub"
mkdir -p "${HF_HUB_CACHE}"
# Redirect Pytorch Hub model folder
export TORCH_HOME="$workdir/ComfyUI_MEGAPAK/TorchHome"
mkdir -p "${TORCH_HOME}"

# Relocate python_standalone
# This move is intentional. It will fast-fail if this breaks anything.
mv  "$workdir"/python_standalone  "$workdir"/ComfyUI_MEGAPAK/python_standalone

################################################################################
# ComfyUI main app
git clone https://github.com/comfyanonymous/ComfyUI.git \
    "$workdir"/ComfyUI_MEGAPAK/ComfyUI
# Use latest stable version (has a release tag)
cd "$workdir"/ComfyUI_MEGAPAK/ComfyUI
...
# Custom Nodes
cd "$workdir"/ComfyUI_MEGAPAK/ComfyUI/custom_nodes
...
cp -rf "$workdir"/attachments/. \
    "$workdir"/ComfyUI_MEGAPAK/

du -hd2 "$workdir"/ComfyUI_MEGAPAK

################################################################################
# TAESD model for image on-the-fly preview
cd "$workdir"
$gcs https://github.com/madebyollin/taesd.git
mkdir -p "$workdir"/ComfyUI_MEGAPAK/ComfyUI/models/vae_approx
cp taesd/*_decoder.pth \
    "$workdir"/ComfyUI_MEGAPAK/ComfyUI/models/vae_approx/
rm -rf taesd

# Download models for ReActor
cd "$workdir"/ComfyUI_MEGAPAK/ComfyUI/models
...
cd "$workdir"/ComfyUI_MEGAPAK/ComfyUI/custom_nodes/ComfyUI-Impact-Pack
"$workdir"/ComfyUI_MEGAPAK/python_standalone/python.exe -s -B install.py
cd "$workdir"/ComfyUI_MEGAPAK/ComfyUI/custom_nodes/ComfyUI-Impact-Subpack
"$workdir"/ComfyUI_MEGAPAK/python_standalone/python.exe -s -B install.py

################################################################################
# Run the test (CPU only), also let custom nodes download some models
cd "$workdir"/ComfyUI_MEGAPAK
...
cd "$workdir"/ComfyUI_MEGAPAK/ComfyUI/custom_nodes/ComfyUI-Manager
git reset --hard
git clean -fxd
