...
* 预装了 30+ 常用自定义节点 <<custom-nodes, 【查看详细列表】>>
...
5. （可选）编辑启动脚本，`run_cn.bat` 或 `run_nvidia_gpu.bat`
...
* 编辑 `extra_model_paths.yaml`，其中以 `#` 开头的部分为注释
...
. 如果是较早版本的整合包，没有 `HuggingFaceHub` 文件夹，可以将
`%USERPROFILE%\.cache\huggingface\hub` 下的文件复制或移动到新版的
`HuggingFaceHub` 文件夹下
