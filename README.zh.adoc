...
8. 提示：善加利用 ComfyUI-Manager （ComfyUI 页面右下角 *Manager* 按钮），可用它来安装更多自定义节点，或更新 ComfyUI 及节点，也可以禁用不需要的节点。

9. 提示：点击 ComfyUI 页面右上角的 齿轮（⚙）图标可修改设置，找到 *AGLTranslation* 可以修改语言（默认 English）。

10. 提示：除了绘图模型外，一些自定义节点还需要下载更多模型，比如 3D 模型会用到的
